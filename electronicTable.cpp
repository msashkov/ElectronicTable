...
bool ElectronicTable::foundedNormalText(Node*& root, bool isInPrintAllFunction) {
    if(isInPrintAllFunction && root->key.getValue().length() > 12) {
        std::string value = root->key.getValue().substr(0, 9) + "...";
        std::cout << value;
        return true;
...
bool ElectronicTable::foundedFormula(Node*& root, bool isInPrintAllFunction, int row, int column) {
...
void ElectronicTable::calculateIndexesOfAbsoluteAddress(int& r, int& c, const std::string& value) {
    r = std::stoi(value.substr(1, value.find('C') - 1));
    c = std::stoi(value.substr(value.find('C') + 1));
}

void ElectronicTable::calculateIndexesOfRelativeAddress(int& r, int& c, const std::string& value) {
    r = std::stoi(value.substr(2, value.find('C') - 2));
    c = std::stoi(value.substr(value.find('C') + 2, value.size() - 2));
}

void ElectronicTable::checkForInfinityLoop(int& currentRow, int& currentColumn, int& currentCounter, std::string& value) {
    ...
    while((validate.functionForAbsoluteAddressing(value) || validate.functionForRelativeAddressing(value)) && 
        ...
        if(validate.functionForAbsoluteAddressing(value)) {
            int r,c;
            calculateIndexesOfAbsoluteAddress(r, c, value);
            value = getValue(r, c);
        }

        else if(validate.functionForRelativeAddressing(value)) {
            int r, c;
            calculateIndexesOfRelativeAddress(r, c, value);
...
bool ElectronicTable::helperForPrintValue(Node* root, int row, int column, bool isInPrintAllFunction) {
    ...
    if(root->key.getRow() == row && root->key.getColumn() == column) {
        ...
        if(root->key.getValue()[0] == '\"') {
           bool flag = foundedNormalText(root, isInPrintAllFunction);
           if(flag) return true;
        }

        else if(root->key.getValue()[0] == '=') {
           bool flag = foundedFormula(root, isInPrintAllFunction, row, column);
           if(flag) return true;
        }
            ...
            checkForInfinityLoop(currentRow, currentColumn, currentCounter, value);
        ...
        helperForPrintValue(root->left, row, column, isInPrintAllFunction);
    }

    else if((root->key.getRow() < row) || (root->key.getRow() == row && root->key.getColumn() < column)) {
        helperForPrintValue(root->right, row, column, isInPrintAllFunction);
    }

}

void ElectronicTable::helperForPrintingLargerTableForEXPR(int& digitForRows, int& digitForColumns) {
...
void ElectronicTable::helperForPrintingSmallerTableForEXPR(int& digitForRows, int& digitForColumns) {
    std::cout << "  |";
...
void ElectronicTable::helperForPrintingLargerTableForValue(int& digitForRows, int& digitForColumns) {
    ...
    while(currentMax <= maxColumn) {
        ...
        while(digitForRows <= maxRow) {
            ...
            while(index <= currentMax) {
                std::cout << std::setw(12);
                if(!helperForPrintValue(root, digitForRows, index, true)) {
                    std::cout << " ";
                }
                std::cout << "|";
                index++;
...
void ElectronicTable::helperForPrintingSmallerTableForValue(int& digitForRows, int& digitForColumns) {
    std::cout << "  |";
    for(int i = 0; i <= maxColumn; i++) { 
        std::cout << std::setw(12) << digitForColumns << "|";
        digitForColumns++;
    ...
    for(int i = 0; i <= maxRow; i++) {
        ...
        for(int j = 0; j <= maxColumn; j++) {
            std::cout << std::setw(12);
            if(!helperForPrintValue(root, i, j, true)) {
                std::cout << " ";
            }
            std::cout << "|";
...
void ElectronicTable::helperForPrintAllEXPR(Node*& root) {
    ...
    if(maxColumn > 10) {
        helperForPrintingLargerTableForEXPR(digitForRows, digitForColumns);
    }

    else {
        helperForPrintingSmallerTableForEXPR(digitForRows, digitForColumns);
    }

}

void ElectronicTable::helperForPrintAllValue(Node*& root) {

    int digitForColumns = 0;
    int digitForRows = 0;

    if(maxColumn > 10) {
        helperForPrintingLargerTableForValue(digitForRows, digitForColumns);
    }

    else {
        helperForPrintingSmallerTableForValue(digitForRows, digitForColumns);
    }
...
void ElectronicTable::printValue(int row, int column) {
    helperForPrintValue(root, row, column, false);
}
...
void ElectronicTable::printAllValue() {
    helperForPrintAllValue(root);
}
...
std::string ElectronicTable::calculatingAdresses(int& currentCounter, const std::string& value, std::stack<double>& values, int& r, int& c) {
...
std::string ElectronicTable::differentCasesForResult(std::string& value, std::stack<double>& values, int& row, 
    int& column, int& r, int& c) {
    ...
    if(validate.functionForNormalText(value) && validate.isDigit(value.substr(1, value.size() - 2))) {
        ...
        checkForInfinityLoop(currentRow, currentColumn, currentCounter, value);
        std::string error = calculatingAdresses(currentCounter, value, values, r, c);
...
std::string ElectronicTable::foundedRelativeAddress(const std::vector<std::string>& partsOftheFormula, int i, int row, int column, std::stack<double>& values) {
    ...
    int r, c;
    calculateIndexesOfRelativeAddress(r, c, partsOftheFormula[i]);
    ...
    std::string error = differentCasesForResult(value, values, row, column, r, c);
    if(error == "\"ERROR\"") return error;
...
std::string ElectronicTable::foundedAbsoluteAddress(const std::vector<std::string>& partsOftheFormula, int i, int row, int column, std::stack<double>& values) {
    ...
    int r, c;
    calculateIndexesOfAbsoluteAddress(r, c, partsOftheFormula[i]);
...
void ElectronicTable::commandSET(const std::string& arguments) {
    ...
    int r, c;
    calculateIndexesOfAbsoluteAddress(r, c, address);
...
void ElectronicTable::commandPRINT(const std::string& arguments) {
    ...
    if(typeOfPrint.compare("VAL") == 0) {
        ...
        if(whatToPrint.compare("ALL") == 0) {
            ...
            int r, c;
            calculateIndexesOfAbsoluteAddress(r, c, whatToPrint);
...
void ElectronicTable::commandIncrement(const std::string& arguments) {
    ...
    int r, c;
    calculateIndexesOfAbsoluteAddress(r, c, arguments);
...
void ElectronicTable::commandSUM(const std::string& arguments) {
    ...
    int r1, c1, r2, c2;
    calculateIndexesOfAbsoluteAddress(r1, c1, address1);
    calculateIndexesOfAbsoluteAddress(r2, c2, address2);
